...
class TestAIGenerator:
    ...
    def setup_method(self):
        ...
        self.mock_client.messages.create = Mock(
            side_effect=self.mock_client.messages_create_side_effect
        )
    ...
    def test_generate_response_with_tools_no_tool_use(self):
        ...
        mock_response = MockAnthropicResponse(
            [MockTextContent("Direct response without tools")]
        )
        self.mock_client.set_responses([mock_response])
    ...
    def test_generate_response_with_tool_use_single_tool(self):
        ...
        final_response = MockAnthropicResponse(
            [MockTextContent("Based on the search results, machine learning is...")]
        )
        ...
        tools = [{"name": "search_course_content", "description": "Search tool"}]
        mock_tool_manager = MockToolManager()
        mock_tool_manager.execute_tool_returns[
            "search_course_content_{'query': 'machine learning', 'course_name': 'AI Basics'}"
    ...
    def test_generate_response_with_multiple_tool_uses(self):
        ...
        tool_use_2 = MockToolUseContent(
            "get_course_outline", {"course_name": "Python"}, "tool_2"
        )
        mock_tool_response = MockAnthropicResponse(
        ...
        final_response = MockAnthropicResponse(
            [MockTextContent("Here's information about Python from multiple sources")]
        )
        ...
        result = self.ai_generator.generate_response(
            "Tell me about Python courses", tools=tools, tool_manager=mock_tool_manager
        )
    ...
    def test_handle_tool_execution_message_structure(self):
        ...
        tool_use_content = MockToolUseContent(
            "search_course_content", {"query": "test query"}, "tool_123"
        )
        ...
        mock_tool_manager = MockToolManager()
        mock_tool_manager.execute_tool_returns[
            "search_course_content_{'query': 'test query'}"
        ] = "Tool result content"
    ...
    def test_sequential_tool_calling_two_rounds(self):
        ...
        mock_tool_manager = MockToolManager()
        mock_tool_manager.execute_tool_returns[
            "get_course_outline_{'course_name': 'Python'}"
        ] = "Python course outline with lesson 4: Advanced Topics"
        mock_tool_manager.execute_tool_returns[
            "search_course_content_{'query': 'lesson 4'}"
        ] = "Advanced Python topics content"
        ...
        assert (
            self.mock_client.messages.create.call_count == 3
        )  # 2 tool rounds + 1 final

        # Verify both tools were executed
        assert len(mock_tool_manager.execute_tool_calls) == 2
        tool_names = [
            call["tool_name"] for call in mock_tool_manager.execute_tool_calls
        ]
        assert "get_course_outline" in tool_names
        assert "search_course_content" in tool_names
    ...
    def test_sequential_tool_calling_early_termination(self):
        ...
        direct_response = MockAnthropicResponse(
            [MockTextContent("Direct answer without tools")]
        )
    ...
    def test_sequential_tool_calling_max_rounds_limit(self):
        ...
        # Round 1: tool use
        tool_use_1 = MockToolUseContent(
            "search_course_content", {"query": "test1"}, "tool_1"
        )
        round_1_response = MockAnthropicResponse([tool_use_1], stop_reason="tool_use")

        # Round 2: tool use (should be last due to max_tool_rounds=2)
        tool_use_2 = MockToolUseContent(
            "search_course_content", {"query": "test2"}, "tool_2"
        )
        round_2_response = MockAnthropicResponse([tool_use_2], stop_reason="tool_use")

        # Final response after max rounds reached
        final_response = MockAnthropicResponse(
            [MockTextContent("Final answer after 2 rounds")]
        )
    ...
    def test_sequential_tool_calling_with_tool_failure(self):
        ...
        final_response = MockAnthropicResponse(
            [MockTextContent("Response after tool failure")]
        )

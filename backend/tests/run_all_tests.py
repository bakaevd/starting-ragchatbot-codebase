...
def run_all_tests():
    ...
    try:
        from test_rag_system import run_rag_system_tests, run_integration_tests
...
def analyze_test_results(results: Dict):
    ...
    print("3. Test with a real query: python -c \"from rag_system import RAGSystem; from config import config; rag = RAGSystem(config); print(rag.query('test'))\"")
...
def generate_fix_recommendations(results: Dict) -> List[str]:
    ...
    if "Integration" in results:
        ...
        for failure in int_results.get("failures", []):
            if "MAX_RESULTS is 0" in failure[1]:
                recommendations.append("CRITICAL: Fix config.py - set MAX_RESULTS to 5 instead of 0")
            elif "Config missing attributes" in failure[1]:
                recommendations.append("Fix config.py - ensure all required attributes are present")
    ...
    if "CourseSearchTool" in results and results["CourseSearchTool"].get("failed", 0) > 0:
        recommendations.append("Debug CourseSearchTool.execute() - check vector store integration")
    
    if "AIGenerator" in results and results["AIGenerator"].get("failed", 0) > 0:
        recommendations.append("Debug AIGenerator tool calling - check Anthropic API integration")
